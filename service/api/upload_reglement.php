...
if (isset($_FILES['reglement'])) {
    ...
    $extension = pathinfo($_FILES['reglement']['name'], PATHINFO_EXTENSION);
    $newFileName = 'reglement_club' . ($extension ? '.' . $extension : '');
    $uploadFile = $uploadDir . $newFileName;

    if (move_uploaded_file($_FILES['reglement']['tmp_name'], $uploadFile)) {
